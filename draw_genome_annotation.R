...
for (i in 1:n_chrs){
  ...
  for (p in 1:n_pages){
    ...
    axis(2, at=seq(5,100,5), labels=rev((seq(0,19,1)*50000+1)+page_offset), cex.axis=0.9, tick = FALSE, las=1)
    axis(4, at=seq(5,100,5), labels=as.integer(rev(seq(1,20,1)*50000)+page_offset), cex.axis=0.9, tick = FALSE, las=1)
    ...
    for (g in is_forward) {
      ...
      polygon( forward_x-x_offset, forward_y, col="#7b7b7b")
    }
    ...
    for (g in is_reverse) {
      ...
      polygon( reverse_x-x_offset, reverse_y, col="#cecece")
    }
    # draw rectangles for strandless features, usually this is an error
    for (g in is_strandless) {
      x_offset = cds_x_offset[g]
      rect(cds_starts[g], cds_y_index[g]-offset_height, cds_ends[g], cds_y_index[g]+offset_height, col="#25893a")
    }
